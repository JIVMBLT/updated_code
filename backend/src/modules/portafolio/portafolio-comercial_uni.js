 ...
 *
 * Las fechas de gratuitos y garantia permanecen como datos informativos y no
 * participan en la clasificacion comercial del Dashboard Portafolio.
 */
...
function portafolioFilters_uni(req, alias = 'p') {
  ...
  if (zona) {
    clauses.push(`${alias}.zona_operativa LIKE ?`);
    params.push(zona);
  }
...
async function getPortafolioDashboard_uni(req, res) {
  const filters = portafolioFilters_uni(req, 'p');
  ...
  try {
    ...
    return res.json({
      ...
      distribuciones: {
        ...
        zona: await distBy_uni(`COALESCE(p.zona_operativa, 'Sin zona')`)
      }
    });
  } catch (error) {
